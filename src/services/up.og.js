...
module.exports = (_options,callback) => {
	...
	const validate = () => {
		console.log('cloudCompose:validateArgs');
		if(!options.path) {
			return Promise.reject('Please specify a valid path to your service');
		}
		if(!options.token) {
			return Promise.reject('Please specify a valid access token');
		} else {
